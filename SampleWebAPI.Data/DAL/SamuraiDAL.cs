...
namespace SampleWebAPI.Data.DAL
{
    public class SamuraiDAL : ISamurai
    {
        ...
        public async Task<IEnumerable<Samurai>> GetSamuraiWithSwords()
        {
            ...
            var samurais = await _context.Samurais.Include(s => s.Swords)
                .ThenInclude(s => s.Elements)
                .Include(s => s.Swords).ThenInclude(s => s.SType)
                .OrderBy(s => s.Name).AsNoTracking().ToListAsync();
            return samurais;

        }
        public async Task<IEnumerable<Samurai>> InsertSamuraiWSword(Samurai obj)
        {
